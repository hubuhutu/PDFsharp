...
namespace PdfSharp.X509
{
    /// <summary>
    /// SignedCmsX
    /// 创建人：zhangyang
    /// 创建时间：2020/7/8 9:33:36
    /// </summary>
    public sealed class SignedCmsX
    {
        [SecurityCritical]
        ...
        public SignedCmsX(ContentInfo contentInfo) : this(SubjectIdentifierType.IssuerAndSerialNumber, contentInfo, false) { }

        public SignedCmsX(SubjectIdentifierType signerIdentifierType, ContentInfo contentInfo) : this(signerIdentifierType, contentInfo, false) { }

        public SignedCmsX(ContentInfo contentInfo, bool detached) : this(SubjectIdentifierType.IssuerAndSerialNumber, contentInfo, detached) { }

        [SecuritySafeCritical]
        public SignedCmsX(SubjectIdentifierType signerIdentifierType, ContentInfo contentInfo, bool detached)
        {
            if (contentInfo == null)
        ...
        public int Version
        {
            ...
            get
            {
                ...
                return m_version;// (int)PkcsUtils.GetVersion(m_safeCryptMsgHandle);
            }
        }
        ...
        {
            if (m_safeCryptMsgHandle == null || m_safeCryptMsgHandle.IsInvalid)
                throw new InvalidOperationException(("Cryptography_Cms_MessageNotSigned"));
        ...
        public void ComputeSignature(CmsSigner signer, bool silent)
        {
            ...
            if (ContentInfo.Content.Length == 0)
                throw new CryptographicException(("Cryptography_Cms_Sign_Empty_Content"));

            if (SubjectIdentifierType.NoSignature == signer.SignerIdentifierType)
            {
                if (m_safeCryptMsgHandle != null && !m_safeCryptMsgHandle.IsInvalid)
                    throw new CryptographicException(("Cryptography_Cms_Sign_No_Signature_First_Signer"));
            ...
            {
                throw new InvalidOperationException(("Cryptography_Cms_RecipientCertificateNotFound"));
            ...
            if (X509UtilsX.GetPrivateKeyInfo(X509UtilsX.GetCertContext(signer.Certificate), ref parameters))
            {
        ...
        private unsafe void Sign(CmsSigner signer, bool silent)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        if (!CAPI.CAPISafe.CryptMsgUpdate(safeCryptMsgHandle, this.ContentInfo.Content, (uint)this.ContentInfo.Content.Length, true))
                            throw new CryptographicException(Marshal.GetLastWin32Error());
                    }

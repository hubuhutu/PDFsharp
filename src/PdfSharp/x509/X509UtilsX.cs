...
namespace PdfSharp.X509
{
    ...
    /// X509UtilsX
    /// 创建人：zhangyang
    /// 创建时间：2020/7/8 9:39:04
    /// </summary>
    
    internal class X509UtilsX
    {
        private X509UtilsX() { }
        ...
        private static void AddToStore(SafeCertStoreHandle safeCertStoreHandle, X509Certificate2Collection collection)
        {
            ...
            {
                using (SafeCertContextHandle ctx = X509UtilsX.GetCertContext(x509))
                {
                                               ...
                                               ref SafeCertChainHandle ppChainContext)
        {
            if (pCertContext == null || pCertContext.IsInvalid)
                throw new ArgumentException(("Cryptography_InvalidContextHandle"), "pCertContext");

            SafeCertStoreHandle hCertStore = SafeCertStoreHandle.InvalidHandle;
            if (extraStore != null && extraStore.Count > 0)
                hCertStore = X509UtilsX.ExportToMemoryStore(extraStore);
            ...
            if (applicationPolicy != null && applicationPolicy.Count > 0)
            {
                ...
                applicationPolicyHandle = X509UtilsX.CopyOidsToUnmanagedMemory(applicationPolicy);
            ...
            if (certificatePolicy != null && certificatePolicy.Count > 0)
            {
                ...
                certificatePolicyHandle = X509UtilsX.CopyOidsToUnmanagedMemory(certificatePolicy);
            ...
            *((long*)&ft) = verificationTime.ToFileTime();
            ...
            uint flags = X509UtilsX.MapRevocationFlags(revocationMode, revocationFlag);
                                                      ...
                                                      IntPtr pdwErrorStatus)
        {
            ...
            int hr = X509UtilsX.BuildChain(new IntPtr(CAPI.HCCE_CURRENT_USER),
                                          pCertContext,
                                          extraStore,
                                          applicationPolicy,
                                          certificatePolicy,
